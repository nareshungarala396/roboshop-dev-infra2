...
resource "aws_security_group_rule" "mongodb_user" {
   type              = "ingress"
   security_group_id = local.mongodb_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.user_sg_id # bastion_sg_id SG ID
   from_port         = 27017
   protocol          = "tcp"
   to_port           = 27017
}
...
resource "aws_security_group_rule" "redis_cart" {
   type              = "ingress"
   security_group_id = local.redis_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.cart_sg_id # bastion_sg_id SG ID
   from_port         = 6379
   protocol          = "tcp"
   to_port           = 6379
}
...
resource "aws_security_group_rule" "mysql_shipping" {
   type              = "ingress"
   security_group_id = local.mysql_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.shipping_sg_id # bastion_sg_id SG ID
   from_port         = 3306
   protocol          = "tcp"
   to_port           = 3306
}
...
resource "aws_security_group_rule" "rabbitmq_payment" {
   type              = "ingress"
   security_group_id = local.rabbitmq_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.payment_sg_id # bastion_sg_id SG ID
   from_port         = 5672
   protocol          = "tcp"
   to_port           = 5672
}
...
resource "aws_security_group_rule" "user_bastion" {
   type              = "ingress"
   security_group_id = local.user_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.bastion_sg_id # bastion_sg_id SG ID
   from_port         = 22
   protocol          = "tcp"
   to_port           = 22
...
resource "aws_security_group_rule" "user_backend_alb" {
   type              = "ingress"
   security_group_id = local.user_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.backend_alb_sg_id # bastion_sg_id SG ID
   from_port         = 8080
   protocol          = "tcp"
   to_port           = 8080
}
...
resource "aws_security_group_rule" "cart_bastion" {
   type              = "ingress"
   security_group_id = local.cart_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.bastion_sg_id # bastion_sg_id SG ID
   from_port         = 22
   protocol          = "tcp"
   to_port           = 22
...
resource "aws_security_group_rule" "payment_bastion" {
   type              = "ingress"
   security_group_id = local.payment_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.bastion_sg_id # bastion_sg_id SG ID
   from_port         = 22
   protocol          = "tcp"
   to_port           = 22
...
resource "aws_security_group_rule" "shipping_bastion" {
   type              = "ingress"
   security_group_id = local.shipping_sg_id # backend_alb_sg_id SG ID
   source_security_group_id = local.bastion_sg_id # bastion_sg_id SG ID
   from_port         = 22
   protocol          = "tcp"
   to_port           = 22
